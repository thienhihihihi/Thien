 ...
 *  
 */
void print_free_list(struct vm_rg_struct *free_list) {
    printf("Current free_list:\n");
    while (free_list) {
        printf("[Start: %d, End: %d] -> ", free_list->rg_start, free_list->rg_end);
...
int __free(struct pcb_t *caller, int rgid) {
  ...
  printf("data voi rgid: %d la start:%d va end la:%d ",rgid,symrg->rg_start,symrg->rg_end);
    if (!symrg) {
