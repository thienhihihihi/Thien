...
while (pgit < pgnum && fpit != NULL) {
    ...
    printf("TAO O DAY NE: %d",*pte);
    pte_set_fpn(pte, fpit->fpn);
    printf("TAO O DAY NE: %08x",*pte);
...
int alloc_pages_range(struct pcb_t *caller, int req_pgnum, struct framephy_struct** frm_lst)
{
  ...
  printf("Successfully allocated %d pages.\n", req_pgnum);
   // In danh sách frm_lst
    printf("Allocated frames (frm_lst):\n");
...
int init_mm(struct mm_struct *mm, struct pcb_t *caller)
{
    ...
    vma1->vm_id = 1;
    vma1->vm_start = PAGING_HEAP_START;
    vma1->vm_end = PAGING_HEAP_START;
